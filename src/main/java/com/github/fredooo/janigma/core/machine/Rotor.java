...
public class Rotor {
	...
	/**
	 * This method handles the carry over operations of the calculations in {@link Enigma#use}.
	...
	public static Rotor[] createM3Rotors() {
		Rotor[] result = new Rotor[M3_ROTOR_LABELS.length];
		for (int i = 0; i < result.length; i++) {
			result[i] = createRotor(i);
		}
		return result;
	}

	public static Rotor[] createM4GreekRotors() {
		Rotor[] result = new Rotor[2];
		result[0] = createRotor(M4_GREEK_BETA);
		result[1] = createRotor(M4_GREEK_GAMMA);
		return result;
	}
	...
	public boolean equals(Object other) {
		...
		return otherRotor.type == this.type && otherRotor.name.equals(this.name);
	}
